...
# --- STARTING ANALYSIS FOR PPAR SIGNALING PATHWAY ---
cat("--- STARTING ANALYSIS (N vs D ONLY) FOR PPAR SIGNALING (hsa03320) ---\n")
...
# Define PPAR Pathway IDs
PPAR_PATHWAY_ID <- "path:hsa03320"
PPAR_HSA_ID <- "hsa03320"
FILE_PREFIX <- "PPAR_SIGNALING"

#########################################
## 1. PREPARE KEGG GENE LIST
#########################################
pathway_id <- PPAR_PATHWAY_ID
...
cat("Step 1: Mapped", nrow(kegg_pathway_genes), "PPAR pathway genes.\n")
...
# Filter to Normothermy (N) and Torpor (D)
...
#########################################
## 3. DEFINE GENES & ORDER (UPDATED)
#########################################

# Define custom pillar genes early so they survive the KEGG filter
pillar_info <- data.frame(
  Gene = c("PPARA", "PPARD", "PPARG", "RXRA", "NR1H3",
           "CD36", "FABP1", "FABP3", "SLC27A4", "DBI", "SCP2",
           "ACSL1", "CPT1A", "CPT1B", "CPT2", "ACADL", "ACOX1", "EHHADH", "ACAA1",
           "PLIN1", "PLIN2", "LPL", "ANGPTL4", "ADIPOQ",
           "APOA1", "APOA5", "APOC3", "PLTP",
           "PCK1", "GK", "ME1", "HMGCS2", "SCD", "FADS2"),
  Function = c(rep("PPAR Receptors & Co-regulators", 5),
               rep("Fatty Acid Transport & Binding", 6),
               rep("Fatty Acid Activation & Oxidation", 8),
               rep("Lipid Storage & Adipogenesis", 5),
               rep("Lipoprotein & Cholesterol Metabolism", 4),
               rep("Glucose, Ketone & Unsaturated FA Metabolism", 6))
)
...
for (TISSUE_NAME in levels(metadata$Tissue)) {
  ...
  pv_out <- pathview(gene.data = gene_data_vector, pathway.id = PPAR_HSA_ID, species = "hsa", 
...
################################################################
## 8. MOLECULAR PILLARS: PPAR TORPOR SHIFT WITH RAW P-VALS
################################################################
...
# 3. Calculate Stats mapping RAW p-values for significance overlays
stats_summary <- counts_long %>%
  ...
  # Apply raw significance threshold filtering
...
pillar_colors <- list(
  Function = c(
    "PPAR Receptors & Co-regulators"                 = "#543005", 
    "Fatty Acid Transport & Binding"                = "#8c510a", 
    "Fatty Acid Activation & Oxidation"              = "#bf812d", 
    "Lipid Storage & Adipogenesis"                   = "#01665e", 
    "Lipoprotein & Cholesterol Metabolism"          = "#35978f", 
    "Glucose, Ketone & Unsaturated FA Metabolism"   = "#1a1a1a"
  )
)

# 6. Plot Final Heatmap straight to global figures output directory
tiff(filename = file.path(main_fig_dir, paste0(FILE_PREFIX, "_praw_Heatmap_PPAR.tiff")), 
     width = 10, height = 11, units = "in", res = 300, compression = "lzw")
...
plot_custom_heatmap(diff_matrix,
                    ...
                    main = "PPAR Signaling: Torpor Shift (D-N)\n(* = Raw p < 0.05, Purple = Up in Torpor)",
                    gaps_row = cumsum(table(annotation_row$Function)[unique(annotation_row$Function)]))
dev.off()

cat(paste0("Success! PPAR raw p-value heatmap and audit documents stored in: ", main_fig_dir, "\n"))