...
metadata<-metadata %>% filter(Tissue %in% c("Pect"))
...
write_csv(as.data.frame(norm.counts), here("Outputs", "WGCNA", "Pectoral", "Pect_norm_counts_rlog.csv"))
...
jpeg(here("Outputs", "WGCNA", "Pectoral", "gene_means_histogram.jpg"), width = 1200, height = 900, res = 150) 
...
jpeg(here("Outputs", "WGCNA", "Pectoral", "gene_vars_histogram.jpg"), width = 1200, height = 900, res = 150) 
...
jpeg(here("Outputs", "WGCNA", "Pectoral", "genes_retained_postFiltering.jpg"), width = 1200, height = 900, res = 150) 
...
jpeg(here("Outputs", "WGCNA", "Pectoral", "softPowerThresholding.jpg"), width = 1200, height = 900, res = 150) 
...
#### choosing power of 17 and continuing ####
...
save(bwnet, file = here("Outputs", "WGCNA", "Pectoral", "Pect_bwnet.RData"))
...
table(bwnet$colors)
write.csv(table(bwnet$colors), here("Outputs", "WGCNA", "Pectoral", "Pect.csv"))


# Plot the dendrogram and the module colors before and after merging underneath
jpeg(here("Outputs", "WGCNA", "Pectoral", "ClusterDendrogram.jpg"), width = 1200, height = 900, res = 150) 
...
# Plot heatmap
jpeg(here("Outputs", "WGCNA", "Pectoral", "modulesVsCategoricalTraits.jpg"), width = 1500, height = 1200, res = 150) 
ggplot(plot_df, aes(x = Trait, y = Module, fill = Correlation)) +
  ...
  labs(title = "Module–Metabolic Trait Correlation (p-values with significance)_Pect", fill = "Correlation")
dev.off()
...
# Step 7: Plot heatmap
jpeg(here("Outputs", "WGCNA", "Pectoral", "modulesVsContinuousTraits.jpg"), width = 1500, height = 1200, res = 150) 
ggplot(plot_df, aes(x = Trait, y = Module, fill = Correlation)) +
  geom_tile(color = "white") +
  geom_text(aes(label = label), color = "black", size = 4.5) +
  scale_fill_gradient2(low = "#2166AC", mid = "white", high = "#B2182B", midpoint = 0) +
...
jpeg(here("Outputs", "WGCNA", "Pectoral", "modulesVsMeanVO2.jpg"), width = 1500, height = 1200, res = 150) 
...
jpeg(here("Outputs", "WGCNA", "Pectoral", "modulesVsSurf_Temp.jpg"), width = 1500, height = 1200, res = 150)
...
lmm_results$tissue<-"Pect"
write.csv(lmm_results, here("Outputs", "WGCNA", "Pectoral", "Effects_predictors_moduleEigenGenes.csv"))
...
jpeg(here("Outputs", "WGCNA", "Pectoral", "Effects_predictors_moduleEigenGenes.jpg"), width = 1500, height = 1200, res = 150)
...
jpeg(here("Outputs", "WGCNA", "Pectoral", "VO2 vs Surf_Temp.jpg"), width = 1500, height = 1200, res = 150)
...
jpeg(here("Outputs", "WGCNA", "Pectoral", "FixedEffects_variancePartitioning_EigenGenes.jpg"), 
...
###### Lesssss Go! Let's inspect Pect modules we want carefully. 
...
run_module_pipeline <- function(
    module_name,
    predictor,
    base_dir = here("Outputs", "WGCNA", "Pectoral")
) { 
...
### VO2 --- Turquoise, Tan, Salmon, Purple, MidnightBlue, Brown  ###
run_module_pipeline("Blue", "mean_VO2")

### Surf_Temp - Tan, Salmon, Purple, MidnightBlue, Green ######
run_module_pipeline("Blue", "Surf_Temp")

########### Looking at metabolic genes ######### 

# Set the parent directory containing Module* folders
parent_dir <- here("Outputs", "WGCNA", "Pectoral")
...
write.csv(sig_df, here("Outputs", "WGCNA", "Pectoral", "genes_in_metabolic_pathways_GenesCorrelatedWith_Mean_VO2_FDR_0.05.csv"))
...
# --------------------------
# 2. Genes of interest
metabolic_genes <- read_csv(here("Outputs", "WGCNA", "Pectoral", "genes_in_metabolic_pathways_GenesCorrelatedWith_Mean_VO2_FDR_0.05.csv"))
...
for (i in seq_len(n_chunks)) {
  ...
  print(p)
  ggsave(
    filename = here::here("Outputs", "WGCNA", "Pectoral", paste0("metabolic_genes_chunk_VO2_", i, ".pdf")),
    plot = p,
    width = 14,
    height = 10
  )
...
write.csv(sig_df, here("Outputs", "WGCNA", "Pectoral", "genes_in_metabolic_pathways_GenesCorrelatedWith_Surf_Temp_FDR_0.05.csv"))
...
# --------------------------
# 2. Genes of interest
metabolic_genes <- read_csv(here("Outputs", "WGCNA", "Pectoral", "genes_in_metabolic_pathways_GenesCorrelatedWith_Surf_Temp_FDR_0.05.csv"))
genes_of_interest <- metabolic_genes$Gene
...
for (i in seq_len(n_chunks)) {
  ...
  print(p)  
  ggsave(
    filename = here::here("Outputs", "WGCNA", "Pectoral", paste0("metabolic_genes_chunk_Surf_Temp_", i, ".pdf")),
    plot = p,
    width = 14,
    height = 10
  )
...
write.csv(final_report_sorted, here("Outputs", "WGCNA", "Pectoral", "IntraModuleMembership_Pect.csv"))
