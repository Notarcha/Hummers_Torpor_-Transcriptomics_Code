...
metadata <- metadata %>% filter(Tissue %in% c("Heart"))
...
write_csv(as.data.frame(norm.counts), here("Outputs", "WGCNA", "Heart", "Heart_norm_counts_rlog.csv"))
...
jpeg(here("Outputs", "WGCNA", "Heart", "gene_means_histogram.jpg"), width = 1200, height = 900, res = 150) 
...
jpeg(here("Outputs", "WGCNA", "Heart", "gene_vars_histogram.jpg"), width = 1200, height = 900, res = 150) 
...
jpeg(here("Outputs", "WGCNA", "Heart", "genes_retained_postFiltering.jpg"), width = 1200, height = 900, res = 150) 
...
jpeg(here("Outputs", "WGCNA", "Heart", "softPowerThresholding.jpg"), width = 1200, height = 900, res = 150) 
...
save(bwnet, file = here("Outputs", "WGCNA", "Heart", "Heart_bwnet.RData"))
...
table(bwnet$colors)
write.csv(table(bwnet$colors), here("Outputs", "WGCNA", "Heart", "Heart_module_counts.csv"))

jpeg(here("Outputs", "WGCNA", "Heart", "ClusterDendrogram.jpg"), width = 1200, height = 900, res = 150) 
...
jpeg(here("Outputs", "WGCNA", "Heart", "modulesVsCategoricalTraits.jpg"), width = 1500, height = 1200, res = 150) 
ggplot(plot_df, aes(x = Trait, y = Module, fill = Correlation)) +
  ...
  labs(title = "Module–Metabolic Trait Correlation (p-values with significance)_Heart", fill = "Correlation")
dev.off()
...
jpeg(here("Outputs", "WGCNA", "Heart", "modulesVsContinuousTraits.jpg"), width = 1500, height = 1200, res = 150) 
ggplot(plot_df, aes(x = Trait, y = Module, fill = Correlation)) +
  geom_tile(color = "white") +
  geom_text(aes(label = label), color = "black", size = 4.5) +
  scale_fill_gradient2(low = "#2166AC", mid = "white", high = "#B2182B", midpoint = 0) +
...
jpeg(here("Outputs", "WGCNA", "Heart", "modulesVsMeanVO2.jpg"), width = 1500, height = 1200, res = 150) 
...
jpeg(here("Outputs", "WGCNA", "Heart", "modulesVsSurf_Temp.jpg"), width = 1500, height = 1200, res = 150)
...
lmm_results$tissue <- "Heart"
write.csv(lmm_results, here("Outputs", "WGCNA", "Heart", "Effects_predictors_moduleEigenGenes.csv"))
...
jpeg(here("Outputs", "WGCNA", "Heart", "Effects_predictors_moduleEigenGenes.jpg"), width = 1500, height = 1200, res = 150)
...
jpeg(here("Outputs", "WGCNA", "Heart", "VO2 vs Surf_Temp.jpg"), width = 1500, height = 1200, res = 150)
...
jpeg(here("Outputs", "WGCNA", "Heart", "FixedEffects_variancePartitioning_EigenGenes.jpg"), 
...
run_module_pipeline <- function(
    module_name,
    predictor,
    base_dir = here("Outputs", "WGCNA", "Heart")
) { 
...
### VO2
run_module_pipeline("Salmon", "mean_VO2")
run_module_pipeline("Greenyellow", "mean_VO2")
run_module_pipeline("Royalblue", "mean_VO2")
run_module_pipeline("Darkgreen", "mean_VO2")

### Surf_Temp
run_module_pipeline("Purple", "Surf_Temp")
run_module_pipeline("Greenyellow", "Surf_Temp")
run_module_pipeline("Lightgreen", "Surf_Temp")
run_module_pipeline("Darkred", "Surf_Temp")
run_module_pipeline("Midnightblue", "Surf_Temp")
run_module_pipeline("Red", "Surf_Temp")
run_module_pipeline("Darkgreen", "Surf_Temp")

########### Looking at metabolic genes ######### 
parent_dir <- here("Outputs", "WGCNA", "Heart")
...
write.csv(sig_df, here("Outputs", "WGCNA", "Heart", "genes_in_metabolic_pathways_GenesCorrelatedWith_Mean_VO2_FDR_0.05.csv"))
...
metabolic_genes <- read_csv(here("Outputs", "WGCNA", "Heart", "genes_in_metabolic_pathways_GenesCorrelatedWith_Mean_VO2_FDR_0.05.csv"))
...
for (i in seq_len(n_chunks)) {
  ...
  print(p)
  ggsave(
    filename = here("Outputs", "WGCNA", "Heart", paste0("metabolic_genes_chunk_VO2_", i, ".pdf")),
    plot = p,
    width = 14,
    height = 10
  )
...
write.csv(sig_df, here("Outputs", "WGCNA", "Heart", "genes_in_metabolic_pathways_GenesCorrelatedWith_Surf_Temp_FDR_0.05.csv"))

metabolic_genes <- read_csv(here("Outputs", "WGCNA", "Heart", "genes_in_metabolic_pathways_GenesCorrelatedWith_Surf_Temp_FDR_0.05.csv"))
genes_of_interest <- metabolic_genes$Gene
...
for (i in seq_len(n_chunks)) {
  ...
  print(p)  
  ggsave(
    filename = here("Outputs", "WGCNA", "Heart", paste0("metabolic_genes_chunk_Surf_Temp_", i, ".pdf")),
    plot = p,
    width = 14,
    height = 10
  )
...
TARGET_MODULES <- c("darkgreen", "salmon", "royalblue", "greenyellow", "purple", "lightgreen", "darkred", "midnightblue", "red")
...
write.csv(final_report_sorted, here("Outputs", "WGCNA", "Heart", "IntraModuleMembership_Heart.csv"))
...
pheatmap(
  ...
  filename = here("Outputs", "WGCNA", "Heart", paste0("Heatmap_", TARGET_MODULE, ".pdf")),
  width  = 10,
  height = 12
)