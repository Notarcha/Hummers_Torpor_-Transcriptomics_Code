...
output_dir <- here("Outputs/WGCNA/Gut2")
...
metadata<-metadata %>% filter(Tissue %in% c("Gut2"))
...
write_csv(as.data.frame(norm.counts),here("Outputs/WGCNA/Gut2/Gut2_norm_counts_rlog.csv"))
...
jpeg(here("Outputs/WGCNA/Gut2/gene_means_histogram.jpg"), width = 1200, height = 900, res = 150) 
...
jpeg(here("Outputs/WGCNA/Gut2/gene_vars_histogram.jpg"), width = 1200, height = 900, res = 150) 
...
jpeg(here("Outputs/WGCNA/Gut2/genes_retained_postFiltering.jpg"), width = 1200, height = 900, res = 150) 
...
jpeg(here("Outputs/WGCNA/Gut2/softPowerThresholding.jpg"), width = 1200, height = 900, res = 150) 
...
#### choosing power of 15 and continuing ####
...
save(bwnet, file = here("Outputs/WGCNA/Gut2/Gut2_bwnet.RData"))
...
table(bwnet$colors)
write.csv(table(bwnet$colors),here("Outputs/WGCNA/Gut2.csv"))


# Plot the dendrogram and the module colors before and after merging underneath
jpeg(here("Outputs/WGCNA/Gut2/ClusterDendrogram.jpg"), width = 1200, height = 900, res = 150) 
...
# Plot heatmap
jpeg(here("Outputs/WGCNA/Gut2/modulesVsCategoricalTraits.jpg"), width = 1500, height = 1200, res = 150) 
ggplot(plot_df, aes(x = Trait, y = Module, fill = Correlation)) +
  ...
  labs(title = "Module–Metabolic Trait Correlation (p-values with significance)_Gut2", fill = "Correlation")
dev.off()
...
# Step 7: Plot heatmap
jpeg(here("Outputs/WGCNA/Gut2//modulesVsContinuousTraits.jpg"), width = 1500, height = 1200, res = 150) 
ggplot(plot_df, aes(x = Trait, y = Module, fill = Correlation)) +
  geom_tile(color = "white") +
  geom_text(aes(label = label), color = "black", size = 4.5) +
  scale_fill_gradient2(low = "#2166AC", mid = "white", high = "#B2182B", midpoint = 0) +
...
jpeg(here("Outputs/WGCNA/Gut2/modulesVsMeanVO2.jpg"), width = 1500, height = 1200, res = 150) 
...
jpeg(here("Outputs/WGCNA/Gut2/modulesVsSurf_Temp.jpg"), width = 1500, height = 1200, res = 150)
...
lmm_results$tissue<-"Gut2"
write.csv(lmm_results,here("Outputs/WGCNA/Gut2/Effects_predictors_moduleEigenGenes.csv"))
...
jpeg(here("Outputs/WGCNA/Gut2/Effects_predictors_moduleEigenGenes.jpg"), width = 1500, height = 1200, res = 150)
...
jpeg(here("Outputs/WGCNA/Gut2/VO2 vs Surf_Temp.jpg"), width = 1500, height = 1200, res = 150)
...
jpeg(here("Outputs/WGCNA/Gut2/FixedEffects_variancePartitioning_EigenGenes.jpg"), 
...
###### Lesssss Go! Let's inspect Gut2 modules we want carefully. 
...
run_module_pipeline <- function(
    module_name,
    predictor,
    base_dir = here("Outputs/WGCNA", "Gut2")
) { 
...
### VO2 --- Turquoise, Tan, Salmon, Purple, MidnightBlue, Brown  ###
run_module_pipeline("Magenta", "mean_VO2")


### Surf_Temp - Tan, Salmon, Purple, MidnightBlue, Green ######
run_module_pipeline("Lightyellow", "Surf_Temp")
run_module_pipeline("Lightcyan", "Surf_Temp")
run_module_pipeline("Purple", "Surf_Temp")
run_module_pipeline("Turquoise", "Surf_Temp")

########### Looking at metabolic genes ######### 

# Set the parent directory containing Module* folders
parent_dir <- here("Outputs/WGCNA", "Gut2")
...
write.csv(sig_df,here("Outputs/WGCNA/Gut2/genes_in_metabolic_pathways_GenesCorrelatedWith_Mean_VO2_FDR_0.05.csv"))
...
# # --------------------------
# # 2. Genes of interest
# metabolic_genes <- read_csv(here("Outputs/WGCNA/Gut2/genes_in_metabolic_pathways_GenesCorrelatedWith_VO2_FDR_0.05.csv"))
...
#   print(p)  
#   ggsave(
#     filename = here::here("Outputs/WGCNA/Gut2", paste0("metabolic_genes_chunk_VO2_", i, ".pdf")),
#     plot = p,
#     width = 14,
#     height = 10
#   )
...
write.csv(sig_df,here("Outputs/WGCNA/Gut2/genes_in_metabolic_pathways_GenesCorrelatedWith_Surf_Temp_FDR_0.05.csv"))
...
# --------------------------
# 2. Genes of interest
metabolic_genes <- read_csv(here("Outputs/WGCNA/Gut2/genes_in_metabolic_pathways_GenesCorrelatedWith_Surf_Temp_FDR_0.05.csv"))
...
for (i in seq_len(n_chunks)) {
  ...
  print(p)  
  ggsave(
    filename = here::here("Outputs/WGCNA/Gut2", paste0("metabolic_genes_chunk_Surf_Temp_", i, ".pdf")),
    plot = p,
    width = 14,
    height = 10
  )
...
# ==============================================================
TARGET_MODULES <- c("lightcyan", "lightyellow", "magenta", "purple","turquoise")
...
write.csv(final_report_sorted,here("Outputs/WGCNA/Gut2/IntraModuleMembership_Gut2.csv"))


######### Sample heatmap: genes (Y) x samples (X), ordered by Surf_Temp #########
...
stopifnot(
  exists("bwnet"),
  exists("norm.counts.filtered"),
  exists("combined_metadata")
)

stopifnot(
  all(c("SampleID", "Surf_Temp") %in% colnames(combined_metadata))
)

# --------------------------
# Step 1: Choose module
# --------------------------
TARGET_MODULE <- "lightcyan"
...
# rows = samples, cols = genes
# --------------------------
expr_mat <- norm.counts.filtered[, genes_in_module, drop = FALSE]
...
# --------------------------
# Step 3: Build sample annotation
# (already sample-level in combined_metadata)
# --------------------------
sample_annot <- combined_metadata %>%
  dplyr::select(SampleID, Surf_Temp) %>%
  filter(!is.na(Surf_Temp))

stopifnot(
  nrow(sample_annot) > 1,
  length(unique(sample_annot$SampleID)) == nrow(sample_annot)
)

# --------------------------
# Step 4: Order samples by Surf_Temp (LOW → HIGH)
# --------------------------
sample_annot <- sample_annot %>%
  arrange(Surf_Temp)

# --------------------------
# Step 5: Align expression matrix
# --------------------------
expr_mat <- expr_mat[sample_annot$SampleID, , drop = FALSE]

stopifnot(
  all(rownames(expr_mat) == sample_annot$SampleID)
)
...
# --------------------------
# Step 7: Z-score per gene
# --------------------------
expr_mat_scaled <- t(scale(t(expr_mat_t)))

# --------------------------
# Step 8: Remove zero-variance / non-finite genes
# --------------------------
keep_genes <- apply(expr_mat_scaled, 1, function(x) all(is.finite(x)))
expr_mat_scaled <- expr_mat_scaled[keep_genes, , drop = FALSE]

message(
  "Removed ",
  sum(!keep_genes),
  " genes with zero variance or non-finite values"
)

stopifnot(
  nrow(expr_mat_scaled) > 1,
  all(is.finite(expr_mat_scaled))
)

# --------------------------
# Step 9: Column annotation
# --------------------------
annotation_col <- data.frame(
  Surf_Temp = sample_annot$Surf_Temp,
  row.names = sample_annot$SampleID
)

stopifnot(
  identical(colnames(expr_mat_scaled), rownames(annotation_col))
)
...
jpeg(
  here(
    "Outputs/WGCNA/Gut2",
    paste0("Heatmap_", TARGET_MODULE, "_genes_by_SurfTemp.jpg")
  ),
  width = 2200,
  height = 2600,
  res = 150
...
pheatmap(
  ...
  main = paste(
    "Gut2", TARGET_MODULE,
    "module genes ordered by surface temperature"
  )
)
