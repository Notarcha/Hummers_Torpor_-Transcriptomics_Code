...
metadata<-metadata %>% filter(Tissue %in% c("Liver"))
...
write_csv(as.data.frame(norm.counts),here("Outputs", "WGCNA", "Liver", "Liver_norm_counts_rlog.csv"))
...
jpeg(here("Outputs", "WGCNA", "Liver", "gene_means_histogram.jpg"), width = 1200, height = 900, res = 150) 
...
jpeg(here("Outputs", "WGCNA", "Liver", "gene_vars_histogram.jpg"), width = 1200, height = 900, res = 150) 
...
jpeg(here("Outputs", "WGCNA", "Liver", "genes_retained_postFiltering.jpg"), width = 1200, height = 900, res = 150) 
...
jpeg(here("Outputs", "WGCNA", "Liver", "softPowerThresholding.jpg"), width = 1200, height = 900, res = 150) 
...
#### choosing power of 20 and continuing ####
...
save(bwnet, file = here("Outputs", "WGCNA", "Liver", "Liver_bwnet.RData"))
...
table(bwnet$colors)
write.csv(table(bwnet$colors),here("Outputs", "WGCNA", "Liver.csv"))


# Plot the dendrogram and the module colors before and after merging underneath
jpeg(here("Outputs", "WGCNA", "Liver", "ClusterDendrogram.jpg"), width = 1200, height = 900, res = 150) 
...
# Plot heatmap
jpeg(here("Outputs", "WGCNA", "Liver", "modulesVsCategoricalTraits.jpg"), width = 1500, height = 1200, res = 150) 
ggplot(plot_df, aes(x = Trait, y = Module, fill = Correlation)) +
  ...
  labs(title = "Module–Metabolic Trait Correlation (p-values with significance)_Liver", fill = "Correlation")
dev.off()
...
# Step 7: Plot heatmap
jpeg(here("Outputs", "WGCNA", "Liver", "modulesVsContinuousTraits.jpg"), width = 1500, height = 1200, res = 150) 
ggplot(plot_df, aes(x = Trait, y = Module, fill = Correlation)) +
  geom_tile(color = "white") +
  geom_text(aes(label = label), color = "black", size = 4.5) +
  scale_fill_gradient2(low = "#2166AC", mid = "white", high = "#B2182B", midpoint = 0) +
...
jpeg(here("Outputs", "WGCNA", "Liver", "modulesVsMeanVO2.jpg"), width = 1500, height = 1200, res = 150) 
...
jpeg(here("Outputs", "WGCNA", "Liver", "modulesVsSurf_Temp.jpg"), width = 1500, height = 1200, res = 150)
...
lmm_results$tissue<-"Liver"
write.csv(lmm_results,here("Outputs", "WGCNA", "Liver", "Effects_predictors_moduleEigenGenes.csv"))
...
jpeg(here("Outputs", "WGCNA", "Liver", "Effects_predictors_moduleEigenGenes.jpg"), width = 1500, height = 1200, res = 150)
...
jpeg(here("Outputs", "WGCNA", "Liver", "VO2 vs Surf_Temp.jpg"), width = 1500, height = 1200, res = 150)
...
jpeg(here("Outputs", "WGCNA", "Liver", "FixedEffects_variancePartitioning_EigenGenes.jpg"), 
...
###### Lesssss Go! Let's inspect Liver modules we want carefully. 
### VO2 --- Turquoise, Tan, Salmon, Purple, MidnightBlue, Brown  ###
### Night_Hour - Purple #### 
### Surf_Temp - Tan, Salmon, Purple, MidnightBlue, Green ######


run_module_pipeline <- function(module_name, predictor, base_dir = here("Outputs", "WGCNA", "Liver")) {
...
# sig_df
# 
# write.csv(sig_df,here("./WGCNA/January2026/Liver/genes_in_metabolic_pathways_GenesCorrelatedWith_Mean_VO2_FDR_0.05.csv"))
...
# # --------------------------
# # 2. Genes of interest
# metabolic_genes <- read_csv(here("./WGCNA/January2026/Liver/genes_in_metabolic_pathways_GenesCorrelatedWith_VO2_FDR_0.05.csv"))
...
#   print(p)  
#   ggsave(
#     filename = here::here("WGCNA/January2026/Liver", paste0("metabolic_genes_chunk_VO2_", i, ".pdf")),
#     plot = p,
#     width = 14,
#     height = 10
#   )
...
# Set the parent directory containing Module* folders
parent_dir <- here("Outputs", "WGCNA", "Liver")
...
write.csv(sig_df,here("Outputs", "WGCNA", "Liver", "genes_in_metabolic_pathways_GenesCorrelatedWith_Surf_Temp_FDR_0.05.csv"))
...
# --------------------------
# 2. Genes of interest
metabolic_genes <- read_csv(here("Outputs", "WGCNA", "Liver", "genes_in_metabolic_pathways_GenesCorrelatedWith_Surf_Temp_FDR_0.05.csv"))
...
for (i in seq_len(n_chunks)) {
  ...
  print(p)  
  ggsave(
    filename = here::here("Outputs", "WGCNA", "Liver", paste0("metabolic_genes_chunk_Surf_Temp_", i, ".pdf")),
    plot = p,
    width = 14,
    height = 10
  )
...
write.csv(final_report_sorted,here("Outputs", "WGCNA", "Liver", "IntraModuleMembership_Liver.csv"))
