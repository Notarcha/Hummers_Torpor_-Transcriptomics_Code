...
##### make directories for all the tissues ##### 
base_dir <- "/Users/notarcha/Desktop/PNAS_Code/Outputs/WGCNA"
tissues <- c("Gut1", "Gut2", "Gut3", "Lungs", "Heart", "Liver", "Pectoral")

for (tissue in tissues) {
  dir_path <- file.path(base_dir, tissue)
...
output_dir <- here("Outputs/WGCNA/Gut1")
...
metadata<-metadata %>% filter(Tissue %in% c("Gut1"))
data<-data[,row.names(metadata)]
all(rownames(metadata) == colnames(data))
metadata_surfTemp_mlO2_min_euthTime <- read.csv(
  here("WGCNA_data/metadata_surfTemp_mlO2_min_euthTime.csv"))
...
write_csv(as.data.frame(norm.counts),here("Outputs/WGCNA/Gut1/Gut1_norm_counts_rlog.csv"))
...
jpeg(here("Outputs/WGCNA/Gut1/gene_means_histogram.jpg"), width = 1200, height = 900, res = 150) 
...
jpeg(here("Outputs/WGCNA/Gut1/gene_vars_histogram.jpg"), width = 1200, height = 900, res = 150) 
...
jpeg(here("Outputs/WGCNA/Gut1/genes_retained_postFiltering.jpg"), width = 1200, height = 900, res = 150) 
...
jpeg(here("Outputs/WGCNA/Gut1/softPowerThresholding.jpg"), width = 1200, height = 900, res = 150) 
...
#### choosing power of 10 and continuing ####
...
save(bwnet, file = here("Outputs/WGCNA/Gut1/Gut1_bwnet.RData"))
...
table(bwnet$colors)
write.csv(table(bwnet$colors),here("Outputs/WGCNA/Gut1.csv"))


# Plot the dendrogram and the module colors before and after merging underneath
jpeg(here("Outputs/WGCNA/Gut1/ClusterDendrogram.jpg"), width = 1200, height = 900, res = 150) 
...
# Plot heatmap
jpeg(here("Outputs/WGCNA/Gut1/modulesVsCategoricalTraits.jpg"), width = 1500, height = 1200, res = 150) 
ggplot(plot_df, aes(x = Trait, y = Module, fill = Correlation)) +
  ...
  labs(title = "Module–Metabolic Trait Correlation (p-values with significance)_Gut1", fill = "Correlation")
dev.off()
...
# Step 7: Plot heatmap
jpeg(here("Outputs/WGCNA/Gut1//modulesVsContinuousTraits.jpg"), width = 1500, height = 1200, res = 150) 
ggplot(plot_df, aes(x = Trait, y = Module, fill = Correlation)) +
  geom_tile(color = "white") +
  geom_text(aes(label = label), color = "black", size = 4.5) +
  scale_fill_gradient2(low = "#2166AC", mid = "white", high = "#B2182B", midpoint = 0) +
...
jpeg(here("Outputs/WGCNA/Gut1/modulesVsMeanVO2.jpg"), width = 1500, height = 1200, res = 150) 
...
jpeg(here("Outputs/WGCNA/Gut1/modulesVsSurf_Temp.jpg"), width = 1500, height = 1200, res = 150)
...
lmm_results$tissue<-"Gut1"
write.csv(lmm_results,here("Outputs/WGCNA/Gut1/Effects_predictors_moduleEigenGenes.csv"))
...
jpeg(here("Outputs/WGCNA/Gut1/Effects_predictors_moduleEigenGenes.jpg"), width = 1500, height = 1200, res = 150)
...
jpeg(here("Outputs/WGCNA/Gut1/VO2 vs Surf_Temp.jpg"), width = 1500, height = 1200, res = 150)
...
jpeg(here("Outputs/WGCNA/Gut1/FixedEffects_variancePartitioning_EigenGenes.jpg"), 
...
###### Lesssss Go! Let's inspect Gut1 modules we want carefully. 
### VO2 --- Turquoise, Tan, Salmon, Purple, MidnightBlue, Brown  ###
### Night_Hour - Purple #### 
### Surf_Temp - Tan, Salmon, Purple, MidnightBlue, Green ######


run_module_pipeline <- function(module_name, predictor, base_dir = here("Outputs/WGCNA/Gut1")) {
...
### Surf_Temp - Tan, Salmon, Purple, MidnightBlue, Green ######
run_module_pipeline("Cyan", "Surf_Temp")
run_module_pipeline("Brown", "Surf_Temp")
...
parent_dir <- here("Outputs/WGCNA/Gut1")
...
write.csv(sig_df,here("Outputs/WGCNA/Gut1/genes_in_metabolic_pathways_GenesCorrelatedWith_Mean_VO2_FDR_0.05.csv"))
...
# # --------------------------
# # 2. Genes of interest
# metabolic_genes <- read_csv(here("Outputs/WGCNA/Gut1/genes_in_metabolic_pathways_GenesCorrelatedWith_VO2_FDR_0.05.csv"))
...
#   print(p)  
#   ggsave(
#     filename = here::here("Outputs/WGCNA/Gut1", paste0("metabolic_genes_chunk_VO2_", i, ".pdf")),
#     plot = p,
#     width = 14,
#     height = 10
#   )
...
write.csv(sig_df,here("Outputs/WGCNA/Gut1/genes_in_metabolic_pathways_GenesCorrelatedWith_Surf_Temp_FDR_0.05.csv"))
...
# --------------------------
# 2. Genes of interest
metabolic_genes <- read_csv(here("Outputs/WGCNA/Gut1/genes_in_metabolic_pathways_GenesCorrelatedWith_Surf_Temp_FDR_0.05.csv"))
...
for (i in seq_len(n_chunks)) {
  ...
  print(p)  
  ggsave(
    filename = here::here("Outputs/WGCNA/Gut1", paste0("metabolic_genes_chunk_Surf_Temp_", i, ".pdf")),
    plot = p,
    width = 14,
    height = 10
  )
...
# ==============================================================
TARGET_MODULES <- c("black", "cyan", "brown", "pink")
...
write.csv(final_report_sorted,here("Outputs/WGCNA/Gut1/IntraModuleMembership_Gut1.csv"))
